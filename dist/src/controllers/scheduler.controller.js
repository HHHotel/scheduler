...
angular.module(DEFAULT.MAIN_PKG).controller("schedCtrl", [
    ...
    function ($rootScope, $scope, $Scheduler) {
        ...
        $scope.saveProfile = function () {
            ...
            for (var i = 0; i < $scope.cache.dogProfile.bookings.length; i++) {
                try {
                    var booking = $scope.cache.dogProfile.bookings[i];
        ...
        $scope.displayEvent = function (event) {
            var date = event.date ? new Date(event.date) : null;
            var text = event.text;
            if (date) {
